...
declare type SingleValueFilterOperator = '=' | '>' | '<' | '>=' | '<=' | '<>' | 'like' | '!=' | 'Timespan';
declare type MultiValueFilterOperator = 'in' | 'not in' | 'between';
declare type FilterVar<T> = keyof T | (string & Record<never, never>);
declare type SingleValueFilter<T> = [FilterVar<T>, SingleValueFilterOperator, Value];
declare type MultiValueFilter<T> = [FilterVar<T>, MultiValueFilterOperator, Value[]];
export declare type Filter<T = FrappeDoc<{}>> = SingleValueFilter<T> | MultiValueFilter<T>;
